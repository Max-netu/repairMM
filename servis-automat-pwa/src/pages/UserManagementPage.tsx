...
export default function UserManagementPage() {
  ...
  async function loadUsers() {
    ...
    try {
      ...
      if (funcError || data.error) {
        throw new Error(funcError?.message || data.error?.message || 'Failed to load users');
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingUser) {
        ...
        if (funcError || data.error) {
          throw new Error(funcError?.message || data.error?.message || 'Failed to update user');
        }
      } else {
        // Create new user
        ...
        if (funcError || data.error) {
          throw new Error(funcError?.message || data.error?.message || 'Failed to create user');
        }
      }
  ...
  const handleDeleteUser = async (userToDelete: User) => {
    ...
    try {
      ...
      if (funcError || data.error) {
        throw new Error(funcError?.message || data.error?.message || 'Failed to delete user');
      }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ 
                      ...formData, 
                      role: e.target.value as 'admin' | 'technician' | 'club',
                      club_id: e.target.value === 'club' ? formData.club_id : null
                    })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
